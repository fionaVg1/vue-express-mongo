<template>
    <section class="wh_content" @touchmove="fn">
        ...
        <a href="javascript:;" class="arrow arrow_left" @click="prevSlide">&lt;</a>
        <a href="javascript:;" class="arrow arrow_right" @click="nextSlide">&gt;</a>
    </section>
</template>
...
export default {
  ...
  mounted() {
    ...
    setTimeout(() => {
      //克隆dom
      this.starDom();
      this.dom.transform = `translate3d(0px, 0px, 0px)`;
      this.dom["-webkit-transform"] = `translate3d(0px, 0px, 0px)`;
      this.dom["-ms-transform"] = `translate3d(0px, 0px, 0px)`;
      if (this.autoPlay) {
        this.setTime();
      }
    }, 50);
  ...
  methods: {
    ...
    toPage(type) {
      ...
      this.setTransform((this.index - 1) * -1 * this._width);
      this.t.m = 0;
      this.t.s = -1; //保证下次重新赋值
      if (this.autoPlay) {
        this.setTime();
    ...
    starDom() {
      ...
      var contentWidth = document.getElementsByClassName("wh_content")[0]
        .offsetWidth;
      var slideWidth = document.getElementsByClassName("wh_slide")[0]
        .offsetWidth;
      var marginRight = Number.parseInt(
        window
          .getComputedStyle(document.getElementsByClassName("wh_slide")[0])
          .marginRight.replace("px", "")
      );
      this.slidesLength = Math.ceil(
        SlideDom.length / Math.floor(contentWidth / (slideWidth + marginRight))
      );
...
.wh_content:hover .arrow {
  display: block;
}
.wh_content .arrow:hover {
